...
class Search(Resource):
    ...
    def get(self):
        ...
        r_key='{}->{}'.format(q,order)
        val_list=self.cache_ops.get(r_key)
        if val_list is not None:
            print('present in cache')
            return {'product': val_list}
        params = {"q": q, "rows": rows_count}
        if order.lower() == "asc":
        ...
        for products_counter in range(0, count):
            ...
            prod_desc = (
                prod_list[products_counter].get("productDescription", "").strip()
            )
            ...
            products.append(
                (prod_id, prod_name, prod_title, prod_desc, prod_img, prod_price)
            )
            ...
            data = {
                ...
                "catlevel1Name": prod_cat_lvl1,
                "catlevel2Name": prod_cat_lvl2,
            }
            insert_status = self.db_ops.insert(data)

        # return {"products": products, "categories": categories}
        print('not present in cache')
        self.cache_ops.set(r_key,products)
        print('cache set')
